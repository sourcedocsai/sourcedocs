...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      plansResult,
      // New queries for missing metrics
      ...
      supabaseAdmin.from('generations').select('doc_type, repo_url, source, generation_time_ms, created_at').order('created_at', { ascending: false }).limit(20),
      ...
      // New: Churn risk - Pro users with no recent generations
      supabaseAdmin.from('users').select('id, username, email, upgraded_at').eq('is_pro', true),
      
      // New: Daily signups for time series
    ...
    const genTimes = generationsResult.data?.map((g: any) => g.generation_time_ms).filter(Boolean) || [];
    ...
    // NEW: Average generations per user
    ...
    // NEW: Power users (top 10 by generation count)
    ...
    // NEW: Generation time by doc type
    ...
    // NEW: Daily and Weekly Active Users
    const now = new Date();
    ...
    // NEW: Churn risk - Pro users with no generations in 30+ days
    ...
    // NEW: Limit hitters (users who hit free limit but didn't upgrade)
